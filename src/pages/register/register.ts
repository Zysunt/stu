...
import { IonicPage, NavController, NavParams,ToastController, Header, LoadingController } from 'ionic-angular';
...
super.showToast(this.toastCtrl,'请输入验证码...')
      ...
      this.http.post('http://appv2.situojinfu.com/index/login/register',{mobile:this.telNum,password:this.password},{headers:this.headers}).map(res =>res.json()).subscribe(function(res){
        console.log(res)
...
settime() {
  ...
  if (this.telNum ==undefined) {
    ...
    var that=this
     ...
     var url='http://appv2.situojinfu.com/api/yxsms/send'
